...
# from data_loaders.chemence.chemence_loader import load_excel_file_chemence  # Import the new Chemence loader
...
# from data_loaders.chemence.chemence_db_utils import save_dataframe_to_db as save_chemence_to_db  # Import the new Chemence db utils
...
# # Updated FILE_TYPES including Chemence
# FILE_TYPES = {
#     "Logiquip": "Logiquip",
#     "Cygnus": "Cygnus",
...
#     "Ternio": "Ternio",
#     "Novo": "Novo",
#     "Chemence": "Chemence",  # Add the new Chemence file type
# }
...
#         # For Excel files, we need year and month from session state
#         if filepath.lower().endswith('.xlsx') or filepath.lower().endswith('.xls'):
#             # Check if year and month are selected for Excel files
#             if "summit_selected_year" not in st.session_state or "summit_selected_month" not in st.session_state:
#                 raise ValueError("For Summit Medical Excel files, you must select both a Year and a Month before processing.")
            
#             return load_excel_file_summit_medical(
#                 filepath, 
#                 year=str(st.session_state["summit_selected_year"]),
#                 month=st.session_state["summit_selected_month"]
#             )
#         else:
...
#     elif file_type == "Novo":  # Branch for Novo
#         # Check if year and month are selected for Novo files
#         if "novo_selected_year" not in st.session_state or "novo_selected_month" not in st.session_state:
#             raise ValueError("For Novo Excel files, you must select both a Year and a Month before processing.")
        
#         return load_excel_file_novo(
#             filepath,
#             year=str(st.session_state["novo_selected_year"]),
#             month=st.session_state["novo_selected_month"]
#         )
#     elif file_type == "Chemence":  # Branch for Chemence
#         # No year/month selection needed as the data is already in the file
...
#     if file_type == "Novo":
#         # List of columns to check for Novo files
#         columns_to_check = [
#             "Customer Number",
#             "Qty Shipped",
#             "Unit Price", 
...
#             # Only check the specified columns for Novo files
...
#         # List of columns to check for Chemence files
#         columns_to_check = [
#             "Source",
#             "Commission Date",
#             "Commission Date YYYY",
...
#         for row_idx, row in df.iterrows():
#             # Only check the specified columns for Chemence files
#             blank_columns = row[existing_columns][row[existing_columns].isnull() | (row[existing_columns] == "")].index.tolist()
#             if blank_columns:
#                 blank_details.append((row_idx, blank_columns))
#     else:
...
#                 "novo_selected_year",     # Add Novo keys
#                 "novo_selected_month",    # Add Novo keys
#                 "novo_selected_month_num", # Add Novo keys
#                 "ternio_selected_year",     # Add Ternio keys
#                 "ternio_selected_month",    # Add Ternio keys
#                 "ternio_selected_month_num" # Add Ternio keys
#                 # No Chemence keys needed as the data is already in the file
#             ]
...
#             if "novo_selected_year" in st.session_state:
#                 del st.session_state["novo_selected_year"]
#             if "novo_selected_month" in st.session_state:
#                 del st.session_state["novo_selected_month"]
#             if "novo_selected_month_num" in st.session_state:
#                 del st.session_state["novo_selected_month_num"]
#             # No Chemence keys to clear as the data is already in the file
...
#             selected_year = st.selectbox("Select Commission Date Year:", year_options, 
#                                     format_func=lambda x: "Select a year..." if x is None else x)
#             selected_month = st.selectbox("Select Commission Date Month:", month_options,
...
#         # Add year and month selectors for Novo
#         elif selected_file_type == "Novo":
#             import datetime
#             current_year = datetime.datetime.now().year
#             year_options = [None, current_year - 1, current_year, current_year + 1]
#             month_options = [None, "January", "February", "March", "April", "May", "June", 
...
#             selected_year = st.selectbox("Select Commission Date Year:", year_options, 
#                                     format_func=lambda x: "Select a year..." if x is None else x,
#                                     key="novo_year_selector")
#             selected_month = st.selectbox("Select Commission Date Month:", month_options,
#                                         format_func=lambda x: "Select a month..." if x is None else x,
#                                         key="novo_month_selector")
            
#             # Check if both selections are made
#             if selected_year is not None and selected_month is not None:
#                 # Store these in session state for later use
#                 st.session_state["novo_selected_year"] = selected_year
#                 st.session_state["novo_selected_month"] = selected_month
#                 st.session_state["novo_selected_month_num"] = month_options.index(selected_month)  # Adjust for None
#                 st.success("Year and month selected successfully!")
#             else:
#                 # Clear previous selections if either is not selected
#                 if "novo_selected_year" in st.session_state:
...
#                 # Show warning if user has already confirmed a file but hasn't selected both year and month
#                 if "confirmed_file_bytes" in st.session_state and selected_file_type == "Novo":
#                     st.warning("Please select both a year and a month before proceeding.")

#         # Add year and month selectors for Ternio
#         elif selected_file_type == "Ternio":
#             import datetime
#             current_year = datetime.datetime.now().year
#             year_options = [None, current_year - 1, current_year, current_year + 1]
#             month_options = [None, "January", "February", "March", "April", "May", "June", 
...
#             selected_year = st.selectbox("Select Commission Date Year:", year_options, 
#                                     format_func=lambda x: "Select a year..." if x is None else x,
#                                     key="ternio_year_selector")
#             selected_month = st.selectbox("Select Commission Date Month:", month_options,
#                                         format_func=lambda x: "Select a month..." if x is None else x,
#                                         key="ternio_month_selector")
            
#             # Check if both selections are made
#             if selected_year is not None and selected_month is not None:
#                 # Store these in session state for later use
#                 st.session_state["ternio_selected_year"] = selected_year
#                 st.session_state["ternio_selected_month"] = selected_month
#                 st.session_state["ternio_selected_month_num"] = month_options.index(selected_month)  # Adjust for None
#                 st.success("Year and month selected successfully!")
#             else:
#                 # Clear previous selections if either is not selected
#                 if "ternio_selected_year" in st.session_state:
#                     del st.session_state["ternio_selected_year"]
#                 if "ternio_selected_month" in st.session_state:
#                     del st.session_state["ternio_selected_month"]
#                 if "ternio_selected_month_num" in st.session_state:
#                     del st.session_state["ternio_selected_month_num"]
                
#                 # Show warning if user has already confirmed a file but hasn't selected both year and month
#                 if "confirmed_file_bytes" in st.session_state and selected_file_type == "Ternio":
#                     st.warning("Please select both a year and a month before proceeding.")        
#         # No year/month selectors needed for Chemence as the data is already in the file
...
#         st.write("For Summit Medical Excel files, please select the commission report date:")
...
#     # If it's Novo but no date selection
#     if (st.session_state.get("selected_file_type") == "Novo" and 
#         "confirmed_file_bytes" in st.session_state and
#         ("novo_selected_year" not in st.session_state or "novo_selected_month" not in st.session_state)):
#         st.warning("⚠️ Please select both a year and month for Novo before proceeding.")
#         should_stop_processing = True

#     # If it's Ternio but no date selection
#     if (st.session_state.get("selected_file_type") == "Ternio" and 
#         "confirmed_file_bytes" in st.session_state and
#         ("ternio_selected_year" not in st.session_state or "ternio_selected_month" not in st.session_state)):
#         st.warning("⚠️ Please select both a year and month for Ternio before proceeding.")
#         should_stop_processing = True    
#     # No date selection check needed for Chemence as the data is already in the file
...
#         # For Ternio specifically, check if year and month are selected
#         if file_type == "Ternio":
#             if "ternio_selected_year" not in st.session_state or "ternio_selected_month" not in st.session_state:
#                 st.error("For Ternio files, you must select both a Year and a Month before processing.")
#                 return

#             # Add the selected year and month as columns
#             df["Commission Date YYYY"] = str(st.session_state["ternio_selected_year"])  # Convert to string
            
#             # Format month as 01, 02, etc.
#             month_num = st.session_state["ternio_selected_month_num"]
#             df["Commission Date MM"] = f"{month_num:02d}"
            
#             # Create the Commission Date string - ensuring both parts are strings
#             df["Commission Date"] = df["Commission Date YYYY"].astype(str) + "-" + df["Commission Date MM"].astype(str)
...
#                 elif f_type == "Novo":
#                     debug_output.extend(save_novo_to_db(df_data, "master_novo_sales"))
#                 elif f_type == "Chemence":
#                     debug_output.extend(save_chemence_to_db(df_data, "master_chemence_sales"))
...
def sales_data_tab():
    ...
    with col1:
        ...
        # Add year and month selectors for Summit Medical (like other file types)
        elif selected_file_type == "Summit Medical":
            import datetime
            current_year = datetime.datetime.now().year
            year_options = [None, current_year - 1, current_year, current_year + 1]
            month_options = [None, "January", "February", "March", "April", "May", "June", 
            ...
            # Add a note explaining that the date selection is only needed for Excel files
            st.info("Note: Date selection is only needed for Excel (.xlsx) files, not PDF files.")
            ...
            if selected_year is not None and selected_month is not None:
                ...
                st.session_state["summit_selected_year"] = selected_year
                st.session_state["summit_selected_month"] = selected_month
                st.session_state["summit_selected_month_num"] = month_options.index(selected_month)  # Adjust for None
                st.success("Year and month selected successfully!")
            else:
                # Clear previous selections if either is not selected
                if "summit_selected_year" in st.session_state:
                ...
                if "summit_selected_month_num" in st.session_state:
                    del st.session_state["summit_selected_month_num"]      
        # No year/month selectors needed for Cygnus, Logiquip, Inspektor, Quickbooks, and Chemence as the data is already in the file
    ...
    # Special handling for Summit Medical Excel files
    # Check if the confirmed file is a Summit Medical Excel file
    is_summit_excel = False
    if (st.session_state.get("selected_file_type") == "Summit Medical" and 
        "confirmed_file_name" in st.session_state):
        if st.session_state["confirmed_file_name"].lower().endswith(('.xlsx', '.xls')):
            is_summit_excel = True
    
    # Display date selectors for Summit Medical Excel files
    if is_summit_excel and ("summit_selected_year" not in st.session_state or "summit_selected_month" not in st.session_state):
        st.warning("⚠️ Year and month selection is required for Summit Medical Excel files. Please select both before proceeding.")
        should_stop_processing = True
