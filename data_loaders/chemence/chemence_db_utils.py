...
def generate_row_hash(row: pd.Series) -> str:
    ...
    columns_to_hash = ["Source ID", "Account Number", "Part #", "Invoice Date", "Commission Date", "Sales Total"]
...
def save_dataframe_to_db(df: pd.DataFrame, table_name: str = "master_chemence_sales"):
    """
    Save data to the 'master_chemence_sales' table by removing entries based on 'Commission Date MM' and 'Commission Date YYYY'.
