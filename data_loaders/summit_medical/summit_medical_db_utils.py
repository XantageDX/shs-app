...
def validate_dataframe_for_hashing(df: pd.DataFrame) -> bool:
    """
    Validate that the DataFrame contains all columns required for row hashing.
    ...
    """
    required_columns = ["Client Name", "Invoice #", "Item ID", "Sales Rep Name", "Date", "ZIP Code"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    
    if missing_columns:
        print(f"❌ Error: DataFrame is missing columns required for hashing: {', '.join(missing_columns)}")
        return False
    return True
...
def save_dataframe_to_db(df: pd.DataFrame, table_name: str = "master_summit_medical_sales"):
    ...
    # Validate DataFrame for hashing
    if not validate_dataframe_for_hashing(df):
        error_msg = f"❌ Error: DataFrame missing required columns for row hash generation"
