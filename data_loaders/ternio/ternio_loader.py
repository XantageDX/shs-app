...
#     raw_df = pd.read_excel(filepath, header=None, skiprows=5)
...
#         # Add 'Date YYYY' and 'Date MM' columns, handling NaN values
#         df["Date YYYY"] = df["Date"].dt.year.astype("Int64").astype(str)
...
#     # df["Sales Rep Name"] = ""  # Will be selected from dropdown
...
#             payment_date = pd.to_datetime(row["Payment Date"]) if pd.notna(row["Payment Date"]) else None
...
#                     merged_df.loc[i, "Payment Date"] = merged_df.loc[i+1, "Payment Date"]
#                     merged_df.loc[i, "Date YYYY"] = merged_df.loc[i+1, "Date YYYY"]
#                     merged_df.loc[i, "Date MM"] = merged_df.loc[i+1, "Date MM"]
...
#                     merged_df.loc[i, "Invoice Date"] = merged_df.loc[i+2, "Payment Date"]
...
#                 merged_df.loc[i, "Invoice Date"] = merged_df.loc[i+1, "Payment Date"]
...
#     # Reorder columns in the specified sequence (removed Transaction Type columns)
#     ordered_columns = [
#         "Client Name",
#         "Payment Date",
#         "Date YYYY",
...
def load_excel_file_ternio(filepath: str) -> pd.DataFrame:
    ...
    # Rename date columns
    if "Date YYYY" in df.columns:
        df = df.rename(columns={"Date YYYY": "Sales Date YYYY"})
    if "Date MM" in df.columns:
        df = df.rename(columns={"Date MM": "Sales Date MM"})
    ...
    # Add Commission Date columns - these will be populated from the UI selection
    ...
    try:
        ...
        def lookup_sales_rep(row):
            client_name = row["Client Name"]
            payment_date = pd.to_datetime(row["Sales Date"]) if pd.notna(row["Sales Date"]) else None
    ...
    while i < len(merged_df):
        ...
        if pd.notna(merged_df.loc[i, "Client Name"]) and merged_df.loc[i, "Client Name"] != "":
            ...
            if i+2 < len(merged_df):
                ...
                if (merged_df.loc[i+1, "Transaction Type 1"] == "Payment" and 
                    ...
                    # Copy data from Payment row (i+1) to current row (i)
                    merged_df.loc[i, "Transaction Type 1"] = merged_df.loc[i+1, "Transaction Type 1"]
                    merged_df.loc[i, "Sales Date"] = merged_df.loc[i+1, "Sales Date"]
                    merged_df.loc[i, "Sales Date YYYY"] = merged_df.loc[i+1, "Sales Date YYYY"]
                    merged_df.loc[i, "Sales Date MM"] = merged_df.loc[i+1, "Sales Date MM"]
                    merged_df.loc[i, "Paid"] = merged_df.loc[i+1, "Paid"]
                    merged_df.loc[i, "Comm Amount"] = merged_df.loc[i+1, "Comm Amount"]
                    
                    # Copy data from Invoice row (i+2) to current row (i)
                    merged_df.loc[i, "Transaction Type 2"] = merged_df.loc[i+2, "Transaction Type 1"]
                    merged_df.loc[i, "Invoice Date"] = merged_df.loc[i+2, "Sales Date"]
                    merged_df.loc[i, "Memo/Description"] = merged_df.loc[i+2, "Memo/Description"]
                    merged_df.loc[i, "Num"] = merged_df.loc[i+2, "Num"]
                    merged_df.loc[i, "Invoiced"] = merged_df.loc[i+2, "Invoiced"]
        ...
        # Case 3: Empty Client Name and Transaction Type 1 is "Payment"
            ...
            if i > 0 and i+1 < len(merged_df):
                ...
                # Copy data from next row (Invoice)
                merged_df.loc[i, "Transaction Type 2"] = merged_df.loc[i+1, "Transaction Type 1"]
                merged_df.loc[i, "Invoice Date"] = merged_df.loc[i+1, "Sales Date"]
                merged_df.loc[i, "Memo/Description"] = merged_df.loc[i+1, "Memo/Description"]
                merged_df.loc[i, "Num"] = merged_df.loc[i+1, "Num"]
                merged_df.loc[i, "Invoiced"] = merged_df.loc[i+1, "Invoiced"]
